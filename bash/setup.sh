...
if [ ! -f $PIDfpm ]; then
    echo "$(date) Starting php7.0-fpm..."
        echo "$(date) Starting php7.0-fpm..." >> ~/server.log
...
##Fails = http://www.hackthesec.co.in/2016/04/how-to-build-nginx-with-pagespeed-module.html
########## Add rules here
...
echo "$hhvmwithfallback" > /usr/local/nginx/conf/hhvm.conf;
...
sudo add-apt-repository -y 'deb http://mirror.i3d.net/pub/mariadb/repo/10.1/ubuntu $(lsb_release -sc) main' && sudo apt-get update
...
# PHP
sudo apt-get install -y php7.0-fpm php7.0-mysql php7.0-curl
